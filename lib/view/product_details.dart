...
class _ProductDetailsState extends State<ProductDetails> with ChangeNotifier implements ProductDetailContract, Connectivity {
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: NotificationListener<OverscrollIndicatorNotification>(
                      ...
                      child: _product == null ? Container() : Stack(
                        children: [
                                ...
                                SliverToBoxAdapter(
                                  child: Column(
                                    children: <Widget>[
                                      ...
                                      SimilarProducts(_similarProducts, false),
                                    ],
                                  ),
                                ),
                              ],
  ...
  void onSuccess(Product product, List<Product> similarProducts) {
    ...
    if((product.variationType == 0 || product.variationType == 1) && product.variations.length > 0) {
      ...
      if(product.selectedVariation != null) {
        ...
        if(product.selectedVariation.regularPrice == product.selectedVariation.discountPrice) {
          ...
          if(product.selectedVariation.discountType == PriceCalculator.AMOUNT) {

            _strDiscount = "৳" + product.selectedVariation.discountAmount.round().toString();
          }
          else {
      ...
      for(int i=0; i<product.variations.length; i++) {
        ...
        if(product.variations[i] != null && product.variations[i].stock > 0) {
          ...
          if(!variationFound) {
            ...
            product.variation2Values.add(VariationValue2(id: variation.id, name: variation.value2, variation1Name: variation.value1));
          }
        }
      }
    }
      ...
      if(product.sizeInfos[0].regularPrice == product.sizeInfos[0].discountPrice) {
        ...
        if(product.sizeInfos[0].discountType == PriceCalculator.AMOUNT) {
          ...
          _strDiscount = "৳" + product.sizeInfos[0].discountAmount.round().toString();
        }
        else {
  ...
  void onAddedToWishList(BuildContext context) {
    ...
    Scaffold.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(duration: Duration(milliseconds: 1500),
        content: Text(AppLocalization.of(context).getTranslatedValue("added_to_wish_list"))));
  }
  ...
  void onRemovedFromWishList(BuildContext context) {
    ...
    Scaffold.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(duration: Duration(milliseconds: 1500),
        content: Text(AppLocalization.of(context).getTranslatedValue("removed_from_wish_list"))));
  }
  ...
  void _onVariationChanged(Variation variation) {
    ...
    if(_product.selectedVariation != null) {
      ...
      if(_product.selectedVariation.regularPrice == _product.selectedVariation.discountPrice) {
        ...
        if(_product.selectedVariation.discountType == PriceCalculator.AMOUNT) {
          ...
          _strDiscount = "৳" + _product.selectedVariation.discountAmount.round().toString();
        }
        else {
