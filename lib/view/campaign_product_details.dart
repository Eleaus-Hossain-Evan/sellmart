...
class CampaignProductDetails extends StatefulWidget {
  ...
  CampaignProductDetails(this.productSlug, this.campaignSlug, this.campaignEndDate);

  @override
  _CampaignProductDetailsState createState() => _CampaignProductDetailsState();
}

class _CampaignProductDetailsState extends State<CampaignProductDetails> implements ProductDetailContract, Connectivity {
  ...
  Product _product = Product(quantity: 1, images: [], sizes: [], variations: [], variation1Values: [], variation2Values: [],
      featured: false, newArrival: false, topSelling: false, freeDelivery: false);
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _presenter.getProductDetails(context, widget.productSlug, campaignSlug: widget.campaignSlug);
    });
            ...
            return SafeArea(
              child: Column(
                children: <Widget>[
                  ...
                  MyAppBar(AppLocalization.of(context).getTranslatedValue("product_details"),
                    onBackPress: () {
                    ...
                    child: NotificationListener<OverscrollIndicatorNotification>(
                      ...
                      child: Stack(
                        children: [
                                      ...
                                      Container(
                                        ...
                                        decoration: BoxDecoration(
                                          ...
                                          borderRadius: BorderRadius.only(
                                            topRight: Radius.circular(3.125 * SizeConfig.heightSizeMultiplier),
                                            topLeft: Radius.circular(3.125 * SizeConfig.heightSizeMultiplier),
                                          ),
                                        ),
                                        child: Column(
                                          children: <Widget>[

                                            ProductInfo(_product, _strDiscount, _strVat,
                                              addToWishList: () {

                                                _presenter.addToWishList(context, _product.id);
                                              },
                                              removeFromWishList: () {

                                                _presenter.removeFromWishList(context, _product.id);
                                              },
                                            ),

                                            CampaignProductVariationInfo(_product, widget.campaignEndDate),

                                            ProductSizeInfo(_product),

                                            ProductDescription(_product),

                                            ProductRatingReview(context, _product),
                                          ],
                                        ),
                                      ),

                                      SimilarProducts(_similarProducts, true, campaignSlug: widget.campaignSlug,),
                                    ],
                                  ),
                                ),
                              ],
                          ...
                          CampaignProductDetailsFooter(_product, widget.campaignEndDate),
                        ],
                      ),
                    ),
                  ),
  ...
  void onSuccess(Product product, List<Product> similarProducts) {
    ...
    if((product.variationType == 0 || product.variationType == 1) && product.variations.length > 0) {
        ...
        if(product.variations[i] != null && product.variations[i].stock > 0) {
          ...
          for(int j=0; j<product.variation1Values.length; j++) {
            ...
            if(product.variation1Values[j].name == variation.value1) {
              ...
              product.variation2Values.insert((j + 1), VariationValue2(id: variation.id, name: variation.value2, variation1Name: variation.value1));
              variationFound = true;
              break;
            }
          }
          ...
          if(!variationFound) {
            ...
            product.variation1Values.add(VariationValue1(id: variation.id, name: variation.value1));
            product.variation2Values.add(VariationValue2(id: variation.id, name: variation.value2));
          }
        }
      }
    }
    else if(product.variationType == 2 && product.sizeInfos.length > 0 && product.sizeInfos[0].infos.length > 1) {
  ...
  void onAddedToWishList(BuildContext context) {
    ...
    Scaffold.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(content: Text(AppLocalization.of(context).getTranslatedValue("added_to_wish_list"))));
  }
  ...
  void onRemovedFromWishList(BuildContext context) {
    ...
    Scaffold.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(content: Text(AppLocalization.of(context).getTranslatedValue("removed_from_wish_list"))));
  }
        ...
        builder: (BuildContext context) {
          ...
          return MyErrorWidget(
            ...
            onPressed: () {
              ...
              _presenter.getProductDetails(mainContext, widget.productSlug, campaignSlug: widget.campaignSlug);
            },
          );
        }
    );
