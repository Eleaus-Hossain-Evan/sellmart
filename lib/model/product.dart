...
class Product {
  ...
  Product({this.id, this.name, this.thumbnail, this.slug, this.sku, this.price, this.vat, this.freeDelivery,
    this.status, this.forOnline, this.featured, this.newArrival, this.topSelling, this.active,
    this.description, this.vendorId, this.vendorName, this.barCode, this.sizes, this.expireDate,
    this.currentStock, this.images, this.discount, this.category, this.subCategory, this.brand, this.quantity,
    this.selectedSize, this.currentPrice, this.subSubCategory, this.variations, this.selectedVariation,
    this.variation1Values, this.variation2Values, this.campaignOffers, this.buyingPrice, this.isCampaignOffer,
    this.variationOne, this.variationTwo, this.salePrice, this.vendorImage, this.vendorSlug, this.advancePayment,
    this.campaignOfferPrice, this.campaignDiscountType, this.campaignOfferDiscount, this.campaignEndDate,
    this.rating, this.totalRating, this.totalReview, this.ratingAnalytic, this.latestReviews, this.sizeInfos,
    this.variationType, this.selectedSizeItem, this.selectedInfoSizeStock});
  ...
  Product.fromJson(Map<String, dynamic> json) {
    ...
    try {
      campaignOfferDiscount = double.parse(json['camPaignDiscountAmount'].toString());
    }
    catch(error) {}
    ...
    try {
      selectedInfoSizeStock = json['selectedInfoSizeStock'] == null ? 0 : int.parse(json['selectedInfoSizeStock'].toString());
    }
    catch(error) {}
  }
  ...
  toJson() {
    ...
    return {
      "_id" : id ?? "",
      "name" : name ?? "",
      "thumbnail" : thumbnail == null ? "" : thumbnail.split(APIRoute.BASE_URL)[1],
      "slug" : slug ?? "",
      "sku" : sku ?? "",
      ...
      "barcode" : barCode ?? "",
      "sizes" : jsonEncode(sizes.map((size) => size.toJson()).toList()).toString(),
      "variations" : jsonEncode(variations.map((variation) => variation.toJson()).toList()).toString(),
      ...
      "subcategory" : subCategory == null ? SubCategory().toJson() : subCategory.toJson(),
      "brand" : brand == null ? Brand().toJson() : brand.toJson(),
      "quantity" : quantity == null ? "" : quantity.toString(),
      "selectedSize" : selectedSize == null ? Size().toJson() : selectedSize.toJson(),
      "selectedVariation" : selectedVariation == null ? Variation().toJson() : selectedVariation.toJson(),
      "currentPrice" : currentPrice == null ? "" : currentPrice.toString(),
      "advancePayment" : advancePayment == null ? "" : advancePayment.toString(),
      "campaignOffer" : isCampaignOffer == null || isCampaignOffer == false ? "0" : "1",
      ...
      "selectedInfoSizeStock" : selectedInfoSizeStock == null ? "0" : selectedInfoSizeStock.toString(),
    };
  }
}
