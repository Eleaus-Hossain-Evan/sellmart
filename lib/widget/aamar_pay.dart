...
import 'aamar_pay_web_view.dart';
...
class AamarPay<T> extends StatefulWidget {
  ...
  @override
  _AamarPayState<T> createState() => _AamarPayState<T>();
}

class _AamarPayState<T> extends State<AamarPay<T>> {
  ...
  void makePaymentCall() {
    ...
    getPayment().then((value) {
      ...
      Future.delayed(Duration(milliseconds: 200), () async {
        ...
        Route route = MaterialPageRoute(builder: (context) => AamarPayWebView(url));
  ...
  Future getPayment() async {
    ...
    if(response.statusCode == 200) {
      ...
      print(mydata.substring(startIndex + start.length, endIndex));
