...
class CheckoutProductList extends StatefulWidget {
  ...
  CheckoutProductList(this.items, this.removalActive, {this.onQuantityChange, this.deleteItem});

  @override
  _CheckoutProductListState createState() => _CheckoutProductListState();
}

class _CheckoutProductListState extends State<CheckoutProductList> with ChangeNotifier {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        border: Border(
          top: BorderSide(color: Colors.grey[300], width: .45 * SizeConfig.widthSizeMultiplier),
          bottom: BorderSide(color: Colors.grey[300], width: .45 * SizeConfig.widthSizeMultiplier),
        ),
      ),
          ...
          return IntrinsicHeight(
            child: Row(
              ...
              children: [
                        ...
                        Flexible(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(widget.items[index].product.name,
                                maxLines: 2,
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(AppLocalization.of(context).getTranslatedValue("tk") + ". " + widget.items[index].product.currentPrice.round().toString(),
                                    style: Theme.of(context).textTheme.bodyText2.copyWith(
                                      ...
                                      children: <Widget>[
                                        ...
                                        GestureDetector(
                                          ...
                                          onTap: () {
                                            ...
                                            _decreaseQuantity(widget.items[index]);
                                          },
                                          child: Container(
                                            padding: EdgeInsets.all(.15 * SizeConfig.heightSizeMultiplier),
                                            decoration: BoxDecoration(
                                              borderRadius: BorderRadius.circular(.75 * SizeConfig.heightSizeMultiplier),
                                              border: Border.all(color: widget.items[index].product.quantity > 1 ? Theme.of(context).primaryColor : Colors.black26, width: .384 * SizeConfig.widthSizeMultiplier),
                                            ),
                                            child: Icon(Icons.remove,
                                              size: 6 * SizeConfig.widthSizeMultiplier,
                                              color: widget.items[index].product.quantity > 1 ? Theme.of(context).primaryColor : Colors.black26,
                                            ),
                                          ),
                                        ),
                                        ...
                                        Padding(
                                          padding: EdgeInsets.only(
                                            top: .15 * SizeConfig.heightSizeMultiplier,
                                            bottom: .15 * SizeConfig.heightSizeMultiplier,
                                            left: 2.8 * SizeConfig.widthSizeMultiplier,
                                            right: 2.8 * SizeConfig.widthSizeMultiplier,
                                          ),
                                          child: Text(widget.items[index].product.quantity.toString(),
                                        ...
                                        GestureDetector(
                                          ...
                                          child: Container(
                                            ...
                                            decoration: BoxDecoration(
                                              borderRadius: BorderRadius.circular(.75 * SizeConfig.heightSizeMultiplier),
                                              border: Border.all(color: Theme.of(context).primaryColor, width: .384 * SizeConfig.widthSizeMultiplier),
                                            ),
                ...
                Column(
                  children: List.generate(200 ~/ 8, (index) => Expanded(
                    child: Container(
                ...
                Expanded(
                  ...
                  child: Padding(
                    ...
                    child: Center(
                      child: Text(AppLocalization.of(context).getTranslatedValue("tk") + ". " +
                          (widget.items[index].product.currentPrice * widget.items[index].product.quantity).round().toString(),
                        maxLines: 3,
                        overflow: TextOverflow.ellipsis,
  ...
  Future<void> _checkQuantity(CartItem item) async {
    ...
    if((item.product.variationType == 0 || item.product.variationType == 1)) {
      ...
      if(item.product.selectedVariation != null && item.product.selectedVariation.id != null) {
      ...
      if(item.product.selectedInfoSizeStock != null && item.product.quantity < item.product.selectedInfoSizeStock) {
